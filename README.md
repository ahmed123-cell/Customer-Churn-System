...
- **Model Serving**: High-performance REST API built with **FastAPI**. Models are exported to **ONNX** format for fast and efficient CPU inference.
...
- **Containerization**: Fully Dockerized API for easy deployment, using volume mounts for model artifacts to allow hot-swapping models without rebuilding the image.
...
The project includes a `Dockerfile` to package the FastAPI application.
